    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] py-16 px-4 text-center font-[Nunito,sans-serif]">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs sm:max-w-none sm:w-auto">
          <Button type="button" variant="ghost" size="sm" onClick={() => router.back()} className="w-full sm:w-auto">
            Go Back
          </Button>
          <Button type="button" variant="secondary" size="sm" onClick={() => router.push("/organization/members")} className="w-full sm:w-auto">
            View All Members
          </Button>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="text-lg sm:text-xl font-extrabold text-[#1F2937] truncate">Member Details</h1>
              <p className="text-xs sm:text-sm text-[#9CA3AF] mt-0.5">View and manage member information</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <div className="text-center">
                  <div className="mx-auto h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-[#0D9488]/10 flex items-center justify-center mb-3">
                    <span className="text-lg sm:text-xl font-extrabold text-[#0D9488]">{initials}</span>
                  </div>
                  <h2 className="text-base sm:text-lg font-bold text-[#1F2937] break-words">{fullName}</h2>
                ...
                <div className="mt-5 space-y-3 border-t border-gray-100 pt-5">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <span className="text-sm text-[#1F2937] break-all min-w-0">{email}</span>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {emailVerified ? (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700">
                        <CheckCircle className="w-3.5 h-3.5 flex-shrink-0" /> Verified
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-600">
                        <Clock className="w-3.5 h-3.5 flex-shrink-0" /> Unverified
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 sm:p-6">
                <h3 className="text-sm sm:text-base font-bold text-[#1F2937] mb-4">Personal Information</h3>
              ...
              {/* Subscriptions */}
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 sm:p-6">
                <h3 className="text-sm sm:text-base font-bold text-[#1F2937] mb-4">Subscriptions</h3>
                ...
                {member.subscriptions && member.subscriptions.length > 0 ? (
                  <div className="space-y-3">
                    {member.subscriptions.map((subscription) => {
                      ...
                      return (
                        <div key={subscription.id} className="border border-gray-100 rounded-xl p-4 bg-[#F9FAFB]">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              {subscription.plan.description && (
                                <p className="text-xs text-[#9CA3AF] mb-2 leading-relaxed">{subscription.plan.description}</p>
                              )}
              ...
              {member.metadata && Object.keys(member.metadata).length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 sm:p-6">
                  <h3 className="text-sm sm:text-base font-bold text-[#1F2937] mb-4">Additional Information</h3>
                  <div className="divide-y divide-gray-50">
                    {Object.entries(member.metadata).map(([key, value]) => (
                      <div key={key} className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2.5 gap-1 sm:gap-4 text-sm">
