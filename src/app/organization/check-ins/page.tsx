...
export default function OrganizationCheckInPage() {
  ...
  // Count members who have at least one check-in today
  ...
  // Sort members by their latest check-in timestamp
  const recentCheckIns = useMemo(
    () =>
      members?.data
        ?.slice()
        .filter((m) => {
          const latest = getLatestCheckIn(m?.checked_in_at);
          return latest !== null;
        })
        .sort((a, b) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
              ...
              <QRCodeScanner isOpen={isScannerOpen} onOpenChange={setIsScannerOpen} onCheckInSuccess={handleCheckInSuccess} />
              {scanMode === "manual" && (
                <div className="space-y-4 pt-2 border-t border-gray-100">
                  <div>
                    <label className="block text-sm font-semibold text-[#1F2937] mb-1.5">Enter Check-In Code</label>
                    <input
                      type="text"
                      value={manualCode}
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="relative mb-4">
              ...
              />
              {recentSearchQuery && (
                <button onClick={() => setRecentSearchQuery("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#9CA3AF] hover:text-[#1F2937] transition-colors">
                  <X size={13} />
                </button>
              )}
            </div>
            ...
            {filteredRecentCheckIns && filteredRecentCheckIns.length > 0 ? (
              <div className="space-y-3 max-h-[520px] overflow-y-auto">
                {filteredRecentCheckIns.map((checkIn) => {
                  const initials = `${checkIn.user.first_name} ${checkIn.user.last_name}`.split(" ").map((n) => n[0]).join("").toUpperCase();
                  ...
                  return (
                    <div key={checkIn.id} className="bg-[#F9FAFB] border border-gray-100 rounded-lg p-3 space-y-2.5">
                      ...
                      <Button type="button" variant="outline" size="sm" className="w-full" onClick={() => { setScanMode("manual"); setCurrentMember(checkIn.id); }}>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-xs text-[#9CA3AF]">
                  {recentSearchQuery ? `No members match "${recentSearchQuery}"` : "Check-ins will appear here"}
                </p>
              </div>
            )}
          </div>
                ...
                {(["today", "week", "month"] as const).map((period) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      timeFilter === period ? "bg-[#0D9488] text-white shadow-sm" : "bg-[#F9FAFB] border border-gray-100 text-[#9CA3AF] hover:text-[#1F2937]",
                    )}
                  >
                    {period}
                  </button>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredStats?.map((stat, index) => {
                    const initials = `${stat.user.first_name} ${stat.user.last_name}`.split(" ").map((n) => n[0]).join("").toUpperCase();
                      ...
                      <tr key={stat.id} className="hover:bg-[#F9FAFB] transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-semibold text-[#1F2937]">{stat.user.first_name} {stat.user.last_name}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <div className="inline-flex items-center gap-1.5">
                            <span className="text-xl font-extrabold text-[#1F2937]">{checkInCount}</span>
                            {index === 0 && <Award className="w-4 h-4 text-amber-400" />}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-sm text-[#9CA3AF]">{formatTimeAgo(latest)}</p>
                        </td>
                      </tr>
                    );
                  })}
                ...
                return (
                  <div key={stat.id} className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[#1F2937]">{stat.user.first_name} {stat.user.last_name}</p>
                        <p className="text-xs text-[#9CA3AF]">{formatTimeAgo(latest)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-lg font-extrabold text-[#1F2937]">{checkInCount}</span>
                      {index === 0 && <Award className="w-4 h-4 text-amber-400" />}
                    </div>
                  </div>
                );
