        ...
        <section className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="px-5 sm:px-6 py-3 sm:py-4 flex items-center gap-6"
              >
                <div className="h-4 flex-1 bg-gray-100 rounded animate-pulse" />
                {[1, 2, 3].map((j) => (
                  <div
                    key={j}
                    className="w-4 h-4 rounded-full bg-gray-100 animate-pulse mx-auto"
                  />
                ))}
              </div>
            ))}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16 space-y-12 sm:space-y-16 lg:space-y-20">
        ...
        <section className="text-center max-w-2xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-[#9CA3AF] mb-8 sm:mb-10 leading-relaxed px-2 sm:px-0">
            Choose the plan that fits your organisation. Start free and upgrade
            as you grow.
          </p>
            ...
            {groupedPlans.map((plan) => (
              ...
              >
                ...
                <div className="px-5 sm:px-6 pt-8 pb-5 border-b border-gray-100">
                  <h3 className="text-base sm:text-lg font-bold text-[#1F2937] mb-1">
                    {plan.name}
                  </h3>
                  <p className="text-sm text-[#9CA3AF] mb-4 sm:mb-5 leading-relaxed">
                    {plan.description}
                  </p>
                  <div>
                    ...
                    {billingCycle === "annually" && plan.price !== null && (
                      <p className="text-xs text-[#9CA3AF] mt-1">
                        Billed annually at ${Number(plan.price).toFixed(2)}/yr
                        {/* {(Number(plan.price) * 12 * 0.85).toFixed(2)}/yr */}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <section className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 sm:px-6 py-5 sm:py-6 border-b border-gray-100">
            <h2 className="text-lg sm:text-xl font-bold text-[#1F2937] mb-1">
              Compare plans
            </h2>
              ...
              <tbody className="divide-y divide-gray-50">
                {includedFeatures.map((feature, i) => (
                  <tr key={i} className="hover:bg-[#F9FAFB] transition-colors">
                    <td className="px-5 sm:px-6 py-3 sm:py-4 text-sm text-[#1F2937]">
                      {feature}
                    </td>
                    {groupedPlans.map((plan) => (
                      <td
