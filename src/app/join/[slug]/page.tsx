...
export default function JoinPage() {
  ...
  const joinOrganization = async (orgSlug: string) => {
    setJoining(true);
    try {
      ...
      const message = String(err.response?.data?.message ?? "Failed to join organization.");
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-white shadow-xl flex items-center justify-center">
            <div
              ...
              style={{ borderWidth: "3px", borderColor: "#e5e7eb", borderTopColor: "#0D9488" }}
            />
          </div>
          <p className="text-sm font-bold text-white/80 bg-black/10 backdrop-blur-sm px-4 py-2 rounded-full">
            {joining ? `Joining ${organizationName ?? "organization"}...` : "Loading..."}
          </p>
        </div>
      </div>
    );
        ...
        <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-12">
          ...
          >
            ...
            <div className="space-y-1.5">
              <h2 className="text-xl font-extrabold text-[#1F2937]">Invalid Invite Link</h2>
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          >
            ...
            {/* Org icon + text */}
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1} className="text-center mb-8 space-y-4">
              {/* Icon */}
              ...
              <div className="space-y-2">
                ...
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#1F2937] leading-tight">
                  Join <span className="text-[#0D9488]">{organizationName}</span>
                </h1>
                <p className="text-sm text-[#9CA3AF] leading-relaxed max-w-xs mx-auto">
                  Sign in or create a free account to accept your invitation and become a member.
                </p>
              </div>
            </motion.div>

            {/* Divider */}
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2} className="relative mb-6">
            ...
            {/* Actions */}
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={3} className="space-y-3 mb-8">
              <Link href={`/auth/login?redirect=/join/${slug}`} className="block">
                <Button className="w-full group" size="lg">
                  Log in to join
                  <ArrowRight className="w-4 h-4 ml-1.5 group-hover:translate-x-0.5 transition-transform" />
                </Button>
              </Link>
              <Link href={`/auth/register?redirect=/join/${slug}`} className="block">
                <Button variant="outline" className="w-full" size="lg">
                  Create an account
                </Button>
              </Link>
            ...
            {/* Terms */}
            <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={4} className="text-center text-xs text-[#9CA3AF] leading-relaxed">
              By joining, you agree to our{" "}
              <a href="/terms" className="font-semibold text-[#1F2937] hover:text-[#0D9488] transition-colors underline underline-offset-2">
                Terms
              </a>{" "}
              and{" "}
              <a href="/privacy" className="font-semibold text-[#1F2937] hover:text-[#0D9488] transition-colors underline underline-offset-2">
                Privacy Policy
              </a>
            </motion.p>
          </motion.div>
          ...
          >
            <p className="text-xs text-white/60 font-medium">
              Powered by <span className="font-extrabold text-white">ReeTrack</span>
            </p>
          </motion.div>
        </div>
      </div>
