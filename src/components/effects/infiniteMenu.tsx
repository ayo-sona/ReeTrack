...
void main() {
    ...
    // Use object-fit: cover logic - center the image and crop to fill
...
class InfiniteGridMenu {
  ...
  private animate(deltaTime: number) {
    ...
    const scale = 0.35; // Increased from 0.25 to make discs bigger
  ...
  private render() {
    ...
    // Use a teal color that matches the theme
    gl.clearColor(0.051, 0.58, 0.533, 1); // #0D9488 in RGB
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const validItems = items.length ? items : defaultItems;
      const itemIndex = index % validItems.length;
      setActiveItem(validItems[itemIndex]);
    };
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && activeItem.title && (
        <>
          {/* Title - positioned bottom-right with coral color */}
          <div
            className={`
              select-none
              absolute
              ...
              ${
                isMoving
                  ? 'opacity-0 pointer-events-none duration-[100ms] translate-y-4'
                  : 'opacity-100 pointer-events-auto duration-[500ms] translate-y-0'
              }
            `}
          >
            <h3 className="text-[#F06543] font-black text-3xl sm:text-4xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.3)] leading-tight">
              {activeItem.title}
            </h3>
          </div>

          {/* Description - positioned below title with engaging style */}
          <p
            className={`
              select-none
              absolute
