...
function getNextBillingDate(interval: string | null | undefined): string | null {
...
function parseApiError(error: any): string {
  if (error?.response) {
    ...
    if (status === 400) return data.message || "Invalid request. Please check your details.";
...
export function SharedCheckout({ mode, plan, backHref, backLabel = "Back", userEmail }: CheckoutConfig) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (mode === "member") {
        ...
        const { data: { data: { invoice } } } = await apiClient.post(
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            {mode === "member" && plan.organization && (
              ...
              >
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 className="text-sm font-bold text-[#0D9488] uppercase tracking-wide">Organisation</h2>
                </div>
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <div>
                    <p className="text-sm font-bold text-[#1F2937]">{plan.organization.name}</p>
                    {plan.organization.description && (
                      <p className="text-xs text-[#9CA3AF] mt-0.5">{plan.organization.description}</p>
                    )}
                  </div>
                </div>
                {(plan.organization.address || plan.organization.email || plan.organization.phone) && (
                  <div className="border-t border-[#E5E7EB] pt-4 space-y-1">
                    {plan.organization.address && <p className="text-xs text-[#9CA3AF]">{plan.organization.address}</p>}
                    {plan.organization.email   && <p className="text-xs text-[#9CA3AF]">{plan.organization.email}</p>}
                    {plan.organization.phone   && <p className="text-xs text-[#9CA3AF]">{plan.organization.phone}</p>}
                  </div>
                )}
              </motion.div>
            )}
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={3}
              className="bg-white rounded-xl border border-[#E5E7EB] p-6 sm:p-7"
            >
              <h2 className="text-sm font-bold text-[#0D9488] uppercase tracking-wide mb-5">Plan Details</h2>
              <div className="space-y-5">
                <div>
                  <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wide mb-1">Plan</p>
                  <p className="text-lg font-extrabold text-[#1F2937]">{plan.name}</p>
                </div>
                {plan.description && (
                  <div>
                    <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wide mb-1">Description</p>
                    <p className="text-sm text-[#1F2937] leading-relaxed">{plan.description}</p>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wide mb-1">Price</p>
                    <p className="text-2xl font-extrabold text-[#0D9488]">{formatCurrency(plan.price)}</p>
                  </div>
                  {plan.interval && (
                    <div>
                      <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wide mb-1">Billing Period</p>
                      <p className="text-base font-bold text-[#1F2937] capitalize">{plan.interval}</p>
                    </div>
                  )}
                </div>
                ...
                {nextBillingDate && (
                  <div className="bg-[#0D9488]/8 border border-[#0D9488]/20 rounded-lg px-4 py-3">
                    <p className="text-xs font-bold text-[#0D9488] uppercase tracking-wide mb-1">Next Billing Date</p>
                    <p className="text-sm font-bold text-[#0D9488]">{nextBillingDate}</p>
                  </div>
                )}

                {plan.features && plan.features.length > 0 && (
                  <div>
                    <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wide mb-3">What&apos;s Included</p>
                    <ul className="space-y-2">
                      {plan.features.map((f, i) => (
                        <li key={i} className="flex items-start gap-2.5">
                          <Check size={15} className="text-[#0D9488] shrink-0 mt-0.5" />
                          <span className="text-sm text-[#1F2937] leading-relaxed">{f}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
            ...
            >
              ...
              <div>
                <p className="text-sm font-bold text-blue-700 mb-0.5">Secure Payment</p>
                <p className="text-xs text-blue-500 leading-relaxed">
                  Your payment information is encrypted. We use industry-standard security to protect your data.
                </p>
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className="bg-white rounded-xl border-2 border-[#E5E7EB] p-6 sm:p-7 lg:sticky lg:top-8">
              <h2 className="text-sm font-bold text-[#0D9488] uppercase tracking-wide mb-5">Order Summary</h2>

              <div className="space-y-3 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[#9CA3AF]">Plan</span>
                  <span className="text-sm font-semibold text-[#1F2937]">{plan.name}</span>
                </div>
                {plan.interval && (
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-[#9CA3AF]">Billing</span>
                    <span className="text-sm font-semibold text-[#1F2937] capitalize">{plan.interval}</span>
                  </div>
                )}
                {nextBillingDate && (
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-[#9CA3AF]">Next Billing</span>
                    <span className="text-sm font-semibold text-[#1F2937]">{nextBillingDate}</span>
                  </div>
                )}
                <div className="border-t border-[#E5E7EB] pt-3 flex justify-between items-baseline">
                  ...
                  <div className="text-right">
                    <span className="text-xl font-extrabold text-[#0D9488]">{formatCurrency(plan.price)}</span>
                    {plan.interval && (
              ...
              <div className="relative mb-4">
                <div className="absolute inset-[-4px] rounded-xl bg-gradient-to-r from-[#F06543]/40 via-[#F06543]/20 to-[#F06543]/40 blur-[14px] z-0"
                  style={{ opacity: isProcessing || plan.price === null ? 0.2 : 0.6 }}
                />
