...
import { Search, CreditCard, X, CheckCircle, Clock, AlertTriangle, ChevronRight, Zap } from "lucide-react";
...
const STATUS_CONFIG: Record<string, { bg: string; color: string; label: string; dot: string }> = {
  ...
  cancelled: { bg: "#F3F4F6",     color: C.coolGrey,  label: "Cancelled", dot: C.coolGrey },
  canceled:  { bg: "#F3F4F6",     color: C.coolGrey,  label: "Cancelled", dot: C.coolGrey },
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: "center",
        gap: "16px",
        boxShadow: accent ? "0 8px 24px rgba(13,148,136,0.25)" : "0 1px 3px rgba(0,0,0,0.04)",
      }}
    >
      <div style={{
      ...
      <div>
        <p style={{ fontWeight: 400, fontSize: "12px", color: accent ? "rgba(255,255,255,0.7)" : C.coolGrey, textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: "2px" }}>{label}</p>
        <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
          <span style={{ fontWeight: 800, fontSize: "24px", color: accent ? C.white : C.ink, letterSpacing: "-0.5px" }}>{value}</span>
          {sub && <span style={{ fontWeight: 400, fontSize: "13px", color: accent ? "rgba(255,255,255,0.6)" : C.coolGrey }}>{sub}</span>}
        </div>
      </div>
    </motion.div>
  );
      ...
      >
        ...
        <div style={{ padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "16px", gap: "10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
              <div style={{
                ...
                fontFamily: "Nunito, sans-serif", fontWeight: 800, fontSize: "18px", flexShrink: 0,
              }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ fontWeight: 400, fontSize: "12px", color: C.coolGrey, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: "4px", marginBottom: "14px" }}>
            <span style={{ fontWeight: 800, fontSize: "26px", color: dimmed ? C.coolGrey : C.ink, letterSpacing: "-0.5px" }}>
              ₦{sub.plan.price.toLocaleString()}
            </span>
            <span style={{ fontWeight: 400, fontSize: "13px", color: C.coolGrey }}>/{sub.plan.interval}</span>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "5px", marginBottom: "14px" }}>
              {featuresArray.slice(0, 2).map((feature: string, idx: number) => (
                <div key={idx} style={{ display: "flex", alignItems: "center", gap: "7px" }}>
                  ...
                  <span style={{ fontWeight: 400, fontSize: "12px", color: C.inkMid }}>{feature}</span>
                </div>
              ))}
              {featuresArray.length > 2 && (
                <span style={{ fontWeight: 400, fontSize: "12px", color: C.coolGrey, paddingLeft: "19px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: "12px", borderTop: `1px solid ${C.border}` }}>
            <div>
              {isActive && days !== null && (
                <p style={{ fontWeight: expiringSoon ? 600 : 400, fontSize: "12px", color: expiringSoon ? C.coral : C.coolGrey }}>
                  {expiringSoon && <AlertTriangle size={11} style={{ display: "inline", marginRight: "4px", verticalAlign: "middle" }} />}
                  {sub.auto_renew ? "Renews" : "Expires"}{" "}
                  <span style={{ fontWeight: 600, color: expiringSoon ? C.coral : C.inkMid }}>
                    {days <= 0 ? "today" : `in ${days}d`}
                  </span>
                </p>
              )}
              {isPending && (
                <p style={{ fontWeight: 400, fontSize: "12px", color: C.amber, display: "flex", alignItems: "center", gap: "4px" }}>
              ...
              {isExpired && (
                <p style={{ fontWeight: 400, fontSize: "12px", color: C.coral }}>Expired</p>
              )}
            </div>
            <ChevronRight size={16} style={{ color: hovered ? C.teal : C.coolGrey, transition: "color 280ms", flexShrink: 0 }} />
          </div>
        </div>
      </motion.div>
    </Link>
...
function SkeletonCard() {
  return (
    <div style={{ background: C.white, borderRadius: "16px", border: `1px solid ${C.border}`, overflow: "hidden", animation: "pulse 1.5s ease-in-out infinite" }}>
      <div style={{ height: "4px", background: C.border }} />
      <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: "14px" }}>
        <div style={{ display: "flex", gap: "10px" }}>
          <div style={{ width: "44px", height: "44px", borderRadius: "11px", background: C.border }} />
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "6px", paddingTop: "4px" }}>
            <div style={{ height: "14px", width: "60%", borderRadius: "4px", background: C.border }} />
            <div style={{ height: "11px", width: "40%", borderRadius: "4px", background: C.border }} />
          </div>
        </div>
        <div style={{ height: "28px", width: "40%", borderRadius: "6px", background: C.border }} />
        <div style={{ height: "1px", background: C.border }} />
        <div style={{ height: "11px", width: "50%", borderRadius: "4px", background: C.border }} />
      </div>
    </div>
  );
}
...
export default function SubscriptionsPage() {
  ...
  const expiringCount = all.filter((s) => s.status === "active" && isExpiringSoon(s.expires_at)).length;
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.snow, fontFamily: "Nunito, sans-serif", padding: "32px 24px 96px" }}>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        {/* ── Page header ── */}
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0} style={{ marginBottom: "28px" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
            <div>
              <h1 style={{ fontWeight: 900, fontSize: "30px", color: C.ink, letterSpacing: "-0.5px", lineHeight: 1.1 }}>My Subscriptions</h1>
              <p style={{ fontWeight: 400, fontSize: "14px", color: C.coolGrey, marginTop: "5px" }}>
                {isLoading ? "Loading…" : `${all.length} subscription${all.length !== 1 ? "s" : ""} across all plans`}
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {showStats && (
            ...
            >
              <StatCard label="Active" value={activeCount} sub="plans" accent icon={<Zap size={20} />} />
              <StatCard label="Expiring soon" value={expiringCount} sub="this week" icon={<AlertTriangle size={20} />} />
              <StatCard label="Pending" value={pendingCount} sub="awaiting" icon={<Clock size={20} />} />
              <StatCard label="Total" value={all.length} sub="all time" icon={<CreditCard size={20} />} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}
          style={{ background: C.white, borderRadius: "12px", border: `1px solid ${C.border}`, padding: "16px 20px", marginBottom: "20px", display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center" }}
        >
          <div style={{ position: "relative", flex: "1 1 220px" }}>
            ...
            <input
              type="text" placeholder="Search by plan name…"
              value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)}
              style={{
                width: "100%", paddingLeft: "34px", paddingRight: "14px", paddingTop: "9px", paddingBottom: "9px",
                ...
                color: C.ink, background: C.snow, outline: "none",
          ...
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {FILTER_OPTIONS.map((s) => {
              ...
              return (
                ...
                >
                  {cfg && (
                    <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: isActive ? "rgba(255,255,255,0.7)" : cfg.dot, flexShrink: 0 }} />
                  )}
        ...
        {!isLoading && !isEmpty && (
          <motion.p
            ...
            style={{ fontWeight: 400, fontSize: "13px", color: C.coolGrey, marginBottom: "14px", paddingLeft: "2px" }}
          >
            Showing <strong style={{ color: C.ink }}>{filtered.length}</strong> subscription{filtered.length !== 1 ? "s" : ""}
            {statusFilter !== "all" && <> · <span style={{ color: STATUS_CONFIG[statusFilter]?.color }}>{STATUS_CONFIG[statusFilter]?.label}</span></>}
          </motion.p>
        )}
        ...
        {isLoading ? (
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2}>
            <div style={{
              ...
              <h3 style={{ fontWeight: 800, fontSize: "20px", color: C.ink, marginBottom: "8px" }}>
                {searchQuery || statusFilter !== "all" ? "No subscriptions match" : "No subscriptions yet"}
              </h3>
              <p style={{ fontWeight: 400, fontSize: "14px", color: C.coolGrey, marginBottom: "28px", maxWidth: "300px", margin: "0 auto 28px", lineHeight: 1.65 }}>
              ...
              {!searchQuery && statusFilter === "all" && (
                <Button
                  ...
                  style={{ background: C.teal, fontFamily: "Nunito, sans-serif", fontWeight: 700 }}
                >
                  Browse Plans
                </Button>
              )}
