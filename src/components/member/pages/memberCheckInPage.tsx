  ...
  const handleGenerateCode = async () => {
    const generateCode = async (): Promise<CheckInCode> => {
      ...
      try {
        ...
        await apiClient.post(`members/check-in/`, {
    ...
    >
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .code-display { padding: 56px 48px; }
        .code-number { font-size: 72px; letter-spacing: 8px; }
        .action-buttons { flex-direction: row; }
        .empty-state { padding: 80px 48px; }
        ...
        @media (max-width: 600px) {
          .code-display { padding: 40px 24px; }
          .code-number { font-size: 52px; letter-spacing: 5px; }
