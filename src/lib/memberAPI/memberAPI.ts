...
const MOCK_SUBSCRIPTIONS: Subscription[] = [
  {
    ...
    features: ['Access to all equipment', 'Free classes', '24/7 access', 'Personal trainer (2 sessions)'],
    autoRenew: true,
  ...
  },
  {
    ...
    cancelAtPeriodEnd: false,
    features: ['Unlimited yoga classes', 'Meditation sessions', 'Wellness workshops'],
    autoRenew: false,
...
const MOCK_WALLET: Wallet = {
  ...
  accountName: 'John Doe - PayPips',
  bankName: 'Wema Bank (PayPips)',
  createdAt: '2024-01-01T00:00:00Z',
...
const MOCK_REFERRAL: Referral = {
  ...
  shareUrl: 'https://paypips.com/join/JOHN123',
};
...
export const memberApi = {
  ...
  getProfile: async (): Promise<Member> => {
    ...
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_MEMBER), 500));
  },

  updateProfile: async (data: Partial<Member>): Promise<Member> => {
    // TODO: Replace with real API call
    return new Promise((resolve) => setTimeout(() => resolve({ ...MOCK_MEMBER, ...data }), 500));
  },
  ...
  getSubscription: async (id: string): Promise<Subscription> => {
    ...
    return new Promise((resolve, reject) => 
      setTimeout(() => sub ? resolve(sub) : reject(new Error('Subscription not found')), 500)
    );
  },

  pauseSubscription: async (id: string): Promise<Subscription> => {
    const sub = MOCK_SUBSCRIPTIONS.find(s => s.id === id);
    return new Promise((resolve) => setTimeout(() => resolve({ ...sub!, status: 'paused' as const }), 500));
  },

  resumeSubscription: async (id: string): Promise<Subscription> => {
    const sub = MOCK_SUBSCRIPTIONS.find(s => s.id === id);
    return new Promise((resolve) => setTimeout(() => resolve({ ...sub!, status: 'active' as const }), 500));
  },

  cancelSubscription: async (id: string): Promise<Subscription> => {
    const sub = MOCK_SUBSCRIPTIONS.find(s => s.id === id);
    return new Promise((resolve) => setTimeout(() => resolve({ ...sub!, cancelAtPeriodEnd: true }), 500));
  },

  upgradeSubscription: async (id: string, planId: string): Promise<Subscription> => {
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_SUBSCRIPTIONS[0]), 500));
  },

  // Wallet
  getWallet: async (): Promise<Wallet> => {
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_WALLET), 500));
  },
  ...
  topUpWallet: async (amount: number, method: 'card' | 'transfer'): Promise<Transaction> => {
  ...
  getPayments: async (): Promise<Payment[]> => {
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_PAYMENTS), 500));
  },
  ...
  getReferral: async (): Promise<Referral> => {
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_REFERRAL), 500));
  },

  getReferredMembers: async (): Promise<ReferredMember[]> => {
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_REFERRED_MEMBERS), 500));
  },
};